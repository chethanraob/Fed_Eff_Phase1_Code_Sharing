...
app.get('/scorecard', function (req, res) {
	...
	client.connect(function (err, client, done) {
		...
		var query = client.query('SELECT DISTINCT CASE WHEN "Agency" IS NULL THEN ' + nullTreatment + ' ELSE "Agency" end as "Agency","Stage_1_2_GHG_emissions", "Stage_3_GHG_emissions","Reduction_in_Energy_Intensity","Usage_of_renewable_energy" FROM "FedEff"."Data_Directory_v10_Predix_07242017" ORDER BY "Agency" ASC', function (err, result) {
...
app.get('/BuildingMoreInfoBdngLevel/:buildingName', function (req, res) {
	...
	client.connect(function (err, client, done) {
		...
		var query = client.query(' SELECT "Total_Mu_Sigma_Score","Energy_Intensity_Score","Energy_Consumption_Score","Site_Area_Score","Number_of_Buildings","Number_Of_Buildings_Metered_For_Electricity","Site_Sq_ft","Annual_Energy_Use_Intensity_Kbtu_per_Sq_Ft","Peak_kW","Annual_Energy_Spending_Comm","Data_added_on","Address","Utility_Partner","Possible_Contracting_Vehicle","Contact_Name","Contact_Number","Interval_data_availability","Current_Customer" FROM "FedEff"."Data_Directory_v10_Predix_07242017" WHERE "Building_Name" = ' + buildingNameString, function (err, result) {
...
app.get('/chartTable', function (req, res) {
	...
	client.connect(function (err, client, done) {
		...
		var query = client.query(' SELECT "State_Name",COUNT(*) as "Opportunities" FROM "FedEff"."Data_Directory_v10_Predix_07242017" GROUP BY "State_Name" ORDER BY "Opportunities" DESC', function (err, result) {
			var response;
			if (err) {
				return console.error('error running query', err);
			}
...
app.get('/chartTable_Agency', function (req, res) {
	...
	client.connect(function (err, client, done) {
		...
		var query = client.query(' SELECT "Agency",COUNT(*) as "Opportunities" FROM "FedEff"."Data_Directory_v10_Predix_07242017" GROUP BY "Agency" ORDER BY "Opportunities" DESC;', function (err, result) {
			var response;
			if (err) {
				return console.error('error running query', err);
			}
